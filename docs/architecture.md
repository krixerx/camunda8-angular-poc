...
The same boundary extends to **i18n (English + Arabic)**. Strapi's built-in i18n localizes the catalog (`service` content fields per locale, `processDefinitionId` shared) and holds Arabic form strings (`form-translation` collection, joined on the Camunda form id). Camunda Forms stay single-source: English is authored in the deployed `.form` files; for `?locale=ar` the backend's `FormTranslator` overlays the Strapi strings onto the schema at read time — **presentational strings only** (labels, descriptions, placeholders, option labels, text views), never keys, types, validation, or FEEL expressions. Fallback chain per string: Arabic translation → authored English; per catalog entry: Arabic localization → English entry → engine data. App chrome strings (nav, buttons) are developer-owned in `frontend/src/app/core/i18n.ts` — chrome changes on code cadence, not editorial cadence. The locale travels as an explicit query param (`/api/services?locale=ar`, form endpoints likewise); English requests never touch the CMS translation path. RTL is a frontend concern: the language switcher sets `document.dir`, and the stylesheets use CSS logical properties.
